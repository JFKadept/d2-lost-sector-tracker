...
async function main(locale: string) {
  ...
  const lostSectorMode =
    (Object.values(DestinyActivityModeDefinition) as DestinyActivityMode[])
      .find((mode) => mode.friendlyName === "lost_sector");

  const legendLostSectorActivityDefs =
    (Object.values(DestinyActivityDefinition) as DestinyActivity[])
      .filter((activity) =>
  ...
  const lostSectors = legendLostSectorActivityDefs.map(
    lostSectorAdapter(modifierMap),
  );
  // TODO: populate Sanity

  try {
    const { results } = await sanityClient.seedData(
      rewards,
      modifiers,
      lostSectors,
      { returnIds: true },
