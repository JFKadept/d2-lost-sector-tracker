...
import type { JSONWorldContent } from "../types/JSONWorldContent.ts";
...
export class BungieClient extends NetClient {
  ...
  async getManifest() {
    const { body } = await this.get<BungieAPIResponse<DestinyManifest>>(
  ...
  async getJSONWorldContent(locale: string) {
    ...
    const { body } = await this.get<JSONWorldContent>(
      manifest.jsonWorldContentPaths[locale],
    );
    return body;
  }
}
