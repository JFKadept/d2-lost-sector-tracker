...
export class NetClient {
  ...
  private async request<T>(
    ...
    const url = this.defaults.baseURL
      ? new URL(
        this.trimLeadingSlash(pathname),
        `${this.trimTrailingSlash(this.defaults.baseURL)}/`,
      )
      : pathname;

    const urlWithParams = `${url}${
      init?.params ? `?${init.params.toString()}` : ""
    }`;
    ...
    if (!res.ok) {
      throw new Error(
        `${method} to ${urlWithParams} failed with ${res.status} (${res.statusText})`,
      );
    }
  ...
  protected get<T>(
    pathname: string,
    init?: Omit<NetRequest, "method" | "body">,
  ): Promise<NetResponse<T>> {
